...
class GiteaAdapter(GitPlatformAdapter):
    ...
    def __init__(self, host: str, token: str, verify_signature: bool = True):
        ...
        self.host = host
        self.token = token
        self._verify_signature_enabled = verify_signature  # Rename to avoid conflict with method
    ...
    def parse_webhook(self, payload: dict, platform: str = "gitea") -> PRMetadata:
        ...
        if "pull_request" in payload:
            ...
            # Push event - extract PR number from commit message or use default
            try:
                commit_msg = payload.get("commits", [{}])[0].get("message", "")

                # Try to extract PR number from commit message (e.g., "Fixes #123" or "#123")
                pr_match = re.search(r"#(\d+)", commit_msg)
                if pr_match:
                    pr_number = int(pr_match.group(1))
                else:
                    # No PR number in commit message - use 1 as default for push events
                    # Per spec: extract from API or commit message; if unavailable, use valid default
                    pr_number = 1

                # Get base_sha from before field, or use zeros if not present (initial commit)
                before_sha = payload.get("before", "")
                base_sha = before_sha if before_sha and len(before_sha) >= 40 else "0" * 40
